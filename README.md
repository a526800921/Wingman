...
| `aux_tool_stats` | 查询各工具累计调用次数和 token 消耗 | 用量观测、成本估算 | 账单审计、跨进程持久化 |
...
| 文件 | 默认路径 | 环境变量/参数覆盖 |
|---|---|---|
| 反馈 JSONL | `~/.wingman/feedback.jsonl` | `AUX_FEEDBACK_LOG_FILE` |
| 工具统计 JSON | `~/.wingman/tool-stats.json` | `AUX_TOOL_STATS_FILE` |
| 运行日志 | `~/.wingman/wingman.log` | `AUX_LOG_FILE` |
| 反馈汇总报告 | `~/.wingman/feedback-reports/` | `--output` (CLI) |
...
| `AUX_LOG_FILE` | 否 | `~/.wingman/wingman.log` | 设置为 `off` 禁用文件日志 |
