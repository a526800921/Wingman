...
- 文档类型：施工计划
- 状态：候选 / 设计中 / 待实施 / 实施中 / 已完成 / 已替代 / 已合并 / 已废弃
- 负责人：
- 依赖计划：
- 相关 ADR：
- 公开 schema 变化：是 / 否
...
- 普通一次性任务不要使用本模板；跨阶段、公共契约、真实反馈、多计划依赖或会反复修改同一模块的工作才进入计划治理。
- 当前阶段必须写细，后续阶段保持粗粒度。
- 实施前必须固定 Step 0 证据，并确认当前阶段没有未解决阻塞项。
- 新信息改变计划顺序、依赖、公共 API、Schema、状态语义、兼容承诺、失败模式或完成证据时，先更新本计划和 `docs/PLAN_MAP.md`。
- 同一事实只在一个位置定义，其他文档使用链接引用。
...
如果 Step 0 不现实，必须记录原因，并定义替代基线后再改实现。
