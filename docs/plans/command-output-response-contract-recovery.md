...
- Step 0 证据：Round 4 fixture 在旧实现上稳定红灯，在新实现上转绿。
- 验证证据：`npm run build`、`npm test`、`npm run smoke`、Round 4 fixture 真实模型回放均通过；详见 `docs/validation/command-output-round4-replay-2026-06-20.md`。
- 治理证据：`docs/PLAN_MAP.md` 已标记为已完成。

## 测试覆盖率

- `test/command-output-response-contract.test.ts` 覆盖响应契约恢复路径。
- `test/fixture-replay.test.ts` 覆盖 Round 4 fixture replay。
- 测试通过：完成记录中已确认 216 tests、10 smoke、build 和 replay ×3 通过。
